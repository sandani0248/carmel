...
if($full_name == '') {
	$error_messages['last_name'] = "Please enter your Full Name";
	$error = 1;
}
...
if($phone == '') {
	$error_messages['mobile'] = "Please enter your Phone Number";
	$error = 1;
}
...
error_log("\nNew Enquiry | last_name: '{$full_name}', mobile: '{$phone}', email: '{$email}', lead_type: '{$type}', date_captured: '{$date}'\n\n");
...
//$con = mysqli_connect("localhost", "root", "khulja$1m","carmel");
$con = mysqli_connect("localhost", "carmelheights_db", "ranjith2812$$","carmelheights_db");

$sql_query = "INSERT INTO {$table}(last_name, mobile, email, lead_type, ip, page_url, date_captured) VALUES('{$full_name}', '{$phone}', '{$email}', '{$type}', '{$ip}', '{$page_url}', '{$date}')";

if( ! mysqli_query($con, $sql_query)) {
	echo mysql_info() . '<br><br>';
	echo mysql_error();
	ini_set("log_errors", 1);
	$log_name = date('M-d-Y', time());
	ini_set("error_log", "{$log_name}.log");
	error_log("\nError: couldn't save {$type} into DB | mysql_error: " . mysql_error() . " | data: '{$full_name}', '{$phone}', '{$email}', '{$type}', '{$ip}', '{$page_url}', '{$date}'\n");
